...
@include('components.front.page-header', [
    ...
    'subtitle' => $websiteSettings['marketplace_intro'] ?? __('messages.marketplace_subtitle'),
    'meta' => [__('messages.search_products'), __('messages.stock'), __('messages.purchase_ready_details')],
])
@include('components.front.proof-strip', [
    'variant' => 'commerce',
    'items' => [
        ['value' => $products->total(), 'label' => __('messages.products_available')],
        ['value' => request('q') ? __('messages.filtered') : __('messages.all'), 'label' => __('messages.catalog_mode')],
        ['value' => __('messages.cod_short'), 'label' => __('messages.payment_method')],
        ['value' => __('messages.live'), 'label' => __('messages.stock_visibility')],
    ],
])
<section class="max-w-7xl mx-auto px-4 py-8" data-page="marketplace-list">
    <form class="fo-surface p-4 mb-6 max-w-3xl grid md:grid-cols-[1fr_auto] gap-3 items-end" id="marketplace-search">
        <div>
            ...
            <input name="q" value="{{ request('q') }}" class="fo-input mt-1" placeholder="{{ __('messages.marketplace_search_placeholder') }}">
        </div>
        ...
        @forelse($products as $product)
            <a href="{{ route('marketplace.show',$product->slug) }}" class="fo-card fo-card-hover p-4 flex flex-col fo-card-commerce" data-cta="marketplace-product-card">
                <div class="mb-3 h-44 overflow-hidden rounded-xl bg-sand/40 border border-sand/80">
                    @if($product->image_url)<img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="h-full w-full object-cover">@else<div class="h-full w-full flex items-center justify-center text-xs text-charcoal/50">{{ __('messages.no_image') }}</div>@endif
                </div>
                ...
                <div class="mt-2 flex items-center justify-between text-xs text-charcoal/70">
                    ...
                    <span class="fo-chip !py-1">{{ __('messages.curated') }}</span>
                </div>
                <p class="text-xs text-charcoal/65 mt-2">{{ __('messages.product_card_hint') }}</p>
                <span class="text-sm font-semibold text-deepred mt-auto pt-3">{{ __('messages.view') }} →</span>
            </a>
        @empty
            <div class="col-span-full">@include('components.front.empty-state', ['subtitle' => __('messages.marketplace_empty_subtitle')])</div>
        @endforelse
    </div>
